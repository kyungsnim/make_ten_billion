...
## 2021.09.26(일)
- 스크롤 문제 해결 (5개 게시글 로드할 때 가장 하단으로 스크롤이 안 내려가는 문제여서 게시글 로드 개수를 10개로 변경함)

## 2021.09.27(월)
- 카카오톡 공유 문제 해결 (debug에선 되나 release모드에선 안되는 문제)
    - 해결 방법 : release의 키 해시를 찾아서 카카오톡 개발자 사이트에 추가해주었고 플레이스토어의 키해시값도 추가해줌
...
- debug 버전에서 정상작동하던 카카오톡 공유하기가 release